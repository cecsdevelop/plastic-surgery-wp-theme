...
function idml_languages_admin_page() {
    ...
    if (isset($_POST['idml_languages_nonce']) && wp_verify_nonce($_POST['idml_languages_nonce'], 'idml_languages_save')) {
        ...
        if ($new_langs) {
            // No hace falta tocar las reglas de rewrite: las de prefijo de
            // idioma se registran con add_rewrite_rule() en cada `init`, así
            // que un cambio de lista surte efecto en la request siguiente sin
            // flush. Comprobado: ninguna de las reglas persistidas lleva
            // prefijo de idioma.
