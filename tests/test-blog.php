 ...
 */
use pswptInit\Blog\BlogController;
...
$code = fn(string $path) => (string) shell_exec('curl -s -o /dev/null -w "%{http_code}" ' . escapeshellarg('http://localhost:8888/WPfemsculpt' . $path));
// Contexto EN (los permalinks siguen el idioma actual, no un parámetro).
$front_en = fn(string $php) => trim((string) shell_exec(PHP_BINARY . ' -r ' . escapeshellarg('$_SERVER["HTTP_HOST"]="localhost:8888"; $_SERVER["REQUEST_URI"]="/WPfemsculpt/en/"; define("WP_USE_THEMES",false); require "' . ABSPATH . 'wp-load.php"; ' . $php) . ' 2>/dev/null'));
...
try {
    ...
    update_post_meta($p1, pswpt_POST_TRANSLATED_TITLE_META, ['en' => 'Test Blog Post One']);
    update_post_meta($p1, pswpt_post_translated_slug_meta_key('en'), 'test-blog-post-one');
    update_post_meta($p1, pswpt_POST_TRANSLATED_EXCERPT_META, ['es' => 'Resumen uno', 'en' => 'Summary one']);
    update_post_meta($p1, pswpt_POST_TRANSLATED_CONTENT_META, ['es' => ['<h2>Problema</h2><p>Cuerpo ES</p>'], 'en' => ['<h2>Problem</h2><p>Body EN</p>']]);
    ...
    $tools = $front_en('echo (new pswptInit\Blog\BlogController())->tools_html(get_post(' . $p1 . '));');
    check('tools_html EN: vuelta a /en/blog/ y botón compartir con URL, título y etiqueta "copiado"', strpos($tools, '<a class="post-tools__back" href="http://localhost:8888/WPfemsculpt/en/blog/">Back to blog</a>') !== false && strpos($tools, 'data-share-url="http://localhost:8888/WPfemsculpt/en/test-blog-post-one/"') !== false && strpos($tools, 'data-copied-label="Link copied"') !== false && strpos($tools, '<span>Share</span>') !== false);
    ...
    $en = $curl('/en/blog/');
    check('EN: título, fecha y excerpt traducidos, enlaces /en/, paginación /en/blog/page/2/', strpos($en, '<h1 class="hero__title">Blog</h1>') !== false && strpos($en, '>' . $p3_short_en . '</time>') !== false && strpos($en, 'href="http://localhost:8888/WPfemsculpt/en/blog/page/2/">2</a>') !== false);
    $en2 = $curl('/en/blog/page/' . $last . '/');
    check('EN última página resuelve (regla /{lang}/{slug}/page/N/) con la entrada uno traducida', $code('/en/blog/page/' . $last . '/') === '200' && strpos($en2, '>Test Blog Post One</a>') !== false && strpos($en2, 'href="http://localhost:8888/WPfemsculpt/en/test-blog-post-one/"') !== false && strpos($en2, 'Summary one') !== false);
