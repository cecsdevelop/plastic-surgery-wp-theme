...
$created = [];
try {
    echo "1) qué texto pide el theme (intelindev_seo_title_override)\n";
    $svc = wp_insert_post(['post_type' => 'intelindev_service', 'post_title' => 'Servicio Test SEO', 'post_name' => 'servicio-test-seo', 'post_status' => 'publish', 'menu_order' => -60]);
    update_post_meta($svc, INTELINDEV_POST_TRANSLATED_TITLE_META, ['en' => 'Test SEO Service']);
    update_post_meta($svc, intelindev_post_translated_slug_meta_key('en'), 'test-seo-service');
    update_post_meta($svc, INTELINDEV_POST_TRANSLATED_EXCERPT_META, ['es' => 'Resumen SEO ES', 'en' => 'SEO summary EN']);
    update_post_meta($svc, INTELINDEV_POST_TRANSLATED_CONTENT_META, ['es' => ['<p>Cuerpo ES</p>'], 'en' => ['<p>Body EN</p>']]);
    $created[] = $svc;

    check('título del plugin con el título por defecto dentro ⇒ se reemplaza conservando la plantilla', intelindev_seo_apply_title('Servicio Test SEO - Intelindev') === 'Servicio Test SEO - Intelindev'); // en ES (idioma por defecto) no se toca
    check('en ES no se toca la descripción ni se inventa una', intelindev_seo_apply_description('') === '' && intelindev_seo_apply_description('Resumen del plugin') === 'Resumen del plugin');
    ...
    update_post_meta($page, INTELINDEV_POST_TRANSLATED_TITLE_META, ['en' => 'Test SEO Page']);
    update_post_meta($page, intelindev_post_translated_slug_meta_key('en'), 'test-seo-page');
    update_post_meta($page, INTELINDEV_POST_TRANSLATED_CONTENT_META, ['es' => ['<p>ES</p>'], 'en' => ['<p>EN</p>']]);
    $created[] = $page;
    ...
    if ($blog > 0) {
        $blog_en = intelindev_get_post_translated_title(get_post($blog), 'en');
