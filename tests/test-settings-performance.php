<?php
/**
 * pswpt Settings → pestaña Performance (image-optimization.php) y
...
$orig = get_option('pswpt_settings', null);
$attachment_id = 0; $tmp = '';

try {
    echo "1) sanitize: purga de claves heredadas y límites\n";
    update_option('pswpt_settings', array_merge(is_array($orig) ? $orig : [], ['bot_guard_enabled' => 1, 'contact_email' => 'x@y.z', 'archive_thumbs' => 1]));
    $out = pswpt_settings_sanitize(['image_quality' => '150', 'max_image_width' => '100', 'preconnect_hosts' => "https://fonts.gstatic.com\n, https://cdn.example.com/x/y", 'critical_images' => '/a.jpg', 'bot_guard_mode' => 'block']);
    ...
    update_option('pswpt_settings', array_merge(is_array($orig) ? $orig : [], ['image_quality' => 70, 'enable_webp' => 1, 'max_image_width' => 1200, 'preconnect_hosts' => ['https://fonts.gstatic.com']]));
    ...
    update_option('pswpt_settings', array_merge(get_option('pswpt_settings'), ['enable_webp' => 0]));
    check('…y sin mapeo cuando está desactivado', !isset(wp_get_image_editor_output_format('a.jpg', 'image/jpeg')['image/jpeg']));
    update_option('pswpt_settings', array_merge(get_option('pswpt_settings'), ['enable_webp' => 1]));

    echo "3) subida real: 1600×1200 JPEG\n";
    $upload = wp_upload_dir();
    $tmp = trailingslashit($upload['path']) . 'pswpt-test-perf-' . wp_generate_password(6, false) . '.jpg';
    ...
    if ($orig === null) delete_option('pswpt_settings'); else update_option('pswpt_settings', $orig);
    echo "   (limpieza: adjunto de prueba y option pswpt_settings)\n";
}
echo $fails ? "\nHAY FALLOS ($fails)\n" : "\nTODO OK\n";
exit($fails ? 1 : 0);
