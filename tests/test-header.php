<?php
/**
 * Header (Apariencia → pswpt Header): CTA con estilo píldora/ícono,
 ...
 *
 * Guarda ajustes temporales en pswpt_header_settings y los restaura.
 */
...
$orig = get_option('pswpt_header_settings', null);

try {
    echo "1) sanitize\n";
    $out = pswpt_header_settings_sanitize(['cta_type' => 'url', 'cta_url' => 'https://example.com/x', 'cta_style' => 'icon', 'nav_bg_color' => '#a4271c', 'active_bg_color' => '#ebe6e6', 'active_link_color' => 'rgba(8, 8, 8, 1)', 'sticky_enabled' => '1', 'sticky_threshold' => '80']);
    ...
    check('cta_style inválido → text', pswpt_header_settings_sanitize(['cta_style' => 'x'])['cta_style'] === 'text');

    echo "2) CTA resuelto\n";
    update_option('pswpt_header_settings', $out);
    $es = pswpt_get_header_cta('es'); $en = pswpt_get_header_cta('en');
    ...
    update_option('pswpt_header_settings', pswpt_header_settings_sanitize(['cta_type' => 'url', 'cta_url' => 'https://example.com/x', 'cta_style' => 'text']));
    check('estilo texto sin texto → sin CTA (como antes)', pswpt_get_header_cta('es')['type'] === 'none');
    update_option('pswpt_header_settings', $out);
    ...
    check('overrides de la matriz en <head> (barra del menú y chip activo)', strpos($h, '--psw-header-nav-bg:#a4271c') !== false && strpos($h, '--psw-header-active-bg:#ebe6e6') !== false && strpos($h, '--psw-header-active-color:rgba(8, 8, 8, 1)') !== false);
    ...
    check('defaults del diseño aprobado: barra flotante negra translúcida, chip activo claro, footer ink, main deja el hueco', strpos($css, '--psw-header-bg: rgba(0, 0, 0, 0.71);') !== false && strpos($css, '--psw-header-active-bg: var(--psw-surface-2);') !== false && strpos($css, '--psw-footer-bg: var(--psw-color-ink);') !== false && strpos($css, ".site-main {\n  padding-top: var(--psw-header-offset);") !== false && strpos($css, ".site-header--sticky {\n  position: fixed;") !== false);
    ...
    if ($orig === null) delete_option('pswpt_header_settings'); else update_option('pswpt_header_settings', $orig);
