...
try {
    ...
    $_GET['intelindev_form'] = (string) $forms['a']; set_current_screen('edit-' . S::POST_TYPE);
    ob_start(); (new S())->export_button('top'); $btn = ob_get_clean();
    check('botón Exportar CSV con el filtro actual', strpos($btn, 'Exportar CSV') !== false && strpos($btn, 'form=' . $forms['a']) !== false);
    ...
    $sub = fn(string $code) => (string) shell_exec(PHP_BINARY . ' -r ' . escapeshellarg('$_SERVER["HTTP_HOST"]="localhost:8888"; $_SERVER["REQUEST_URI"]="/Intelindev/wp-admin/admin-post.php"; define("WP_USE_THEMES",false); define("WP_ADMIN",true); require "' . ABSPATH . 'wp-load.php"; wp_set_current_user(1); ' . $code) . ' 2>/dev/null');
    $out = $sub('$_GET["form"] = "' . $forms['a'] . '"; $_REQUEST["_wpnonce"] = wp_create_nonce("' . S::EXPORT_ACTION . '"); (new \IntelindevInit\Forms\SubmissionsController())->handle_export();');
    check('con nonce válido devuelve el CSV', strpos($out, "\xEF\xBB\xBFID,Fecha,Formulario") === 0 && substr_count($out, "\n") === 3);
    $bad = $sub('$_GET["form"] = "0"; $_REQUEST["_wpnonce"] = "x"; (new \IntelindevInit\Forms\SubmissionsController())->handle_export();');
    check('nonce inválido → sin CSV', strpos($bad, "ID,Fecha") === false);
    $anon = $sub('wp_set_current_user(0); $_REQUEST["_wpnonce"] = wp_create_nonce("' . S::EXPORT_ACTION . '"); (new \IntelindevInit\Forms\SubmissionsController())->handle_export();');
