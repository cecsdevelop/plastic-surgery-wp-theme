...
try {
    ...
    $h = $curl('/');
    check('styles.css aplica las variables al body y títulos', preg_match('/^body \{[^}]*font-family: var\(--intelindev-font-body\);/m', $css = file_get_contents(dirname(__DIR__) . '/assets/css/styles.css')) === 1 && strpos($css, '--intelindev-font-body: var(--wp--preset--font-family--dm-sans,') !== false && strpos($css, '--intelindev-font-accent: var(--wp--preset--font-family--dm-serif-text,') !== false && preg_match('/h1 em, h2 em, h3 em, h4 em,\n\.accent \{\n  font-family: var\(--intelindev-font-accent\);/', $css) === 1);
    ...
    $h = $curl('/');
    check('override en <head> con las 5 variables', strpos($h, '<style id="intelindev-typography-overrides">:root{--intelindev-font-body:var(--wp--preset--font-family--inter);--intelindev-font-heading:var(--wp--preset--font-family--system);--intelindev-font-accent:var(--wp--preset--font-family--dm-serif-text);--intelindev-font-size-base:18px;--intelindev-font-weight-heading:600;}</style>') !== false);
