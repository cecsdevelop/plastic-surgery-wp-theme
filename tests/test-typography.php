...
$orig_settings = get_option('pswpt_settings', null);
...
try {
    ...
    $fam = pswpt_typography_font_families();
    ...
    check('preload del theme: DM Sans elige la cara normal (variable, contiene 400) y la sirve desde assets/fonts', pswpt_typography_preload_src('dm-sans') === get_template_directory_uri() . '/assets/fonts/dm-sans-variable-latin.woff2');
    $out = pswpt_settings_sanitize(['font_body' => 'system', 'font_heading' => 'no-existe', 'font_accent' => 'dm-serif-text', 'font_size_base' => '30', 'font_weight_heading' => '650', 'font_preload' => '']);
    ...
    $out = pswpt_settings_sanitize(['font_size_base' => '', 'font_weight_heading' => '300', 'font_preload' => '1']);
    ...
    check('sin ajustes: sin override; preload de DM Sans (default del theme); WP imprime los presets y los @font-face de assets/fonts (swap)', strpos($h, 'pswpt-typography-overrides') === false && strpos($h, '<link rel="preload" href="' . get_template_directory_uri() . '/assets/fonts/dm-sans-variable-latin.woff2" as="font" type="font/woff2" crossorigin>') !== false && strpos($h, '--wp--preset--font-family--dm-sans') !== false && strpos($h, '--wp--preset--font-family--system') !== false && substr_count($h, 'font-display:swap;src:url(\'' . get_template_directory_uri() . '/assets/fonts/') === 4);
    ...
    $gs['settings']['typography']['fontFamilies']['custom'] = [[
        ...
        'fontFace' => [
            ['fontFamily' => 'Inter', 'fontStyle' => 'normal', 'fontWeight' => '700', 'src' => ['http://localhost:8888/WPfemsculpt/wp-content/uploads/fonts/inter-700.woff2']],
            ['fontFamily' => 'Inter', 'fontStyle' => 'normal', 'fontWeight' => '400', 'src' => ['http://localhost:8888/WPfemsculpt/wp-content/uploads/fonts/inter-400.woff2']],
        ],
    ]];
    ...
    check('preload elige la cara 400 normal (woff2)', pswpt_typography_preload_src('inter') === 'http://localhost:8888/WPfemsculpt/wp-content/uploads/fonts/inter-400.woff2' && pswpt_typography_preload_src('system') === '');
    update_option('pswpt_settings', ['font_body' => 'inter', 'font_heading' => 'system', 'font_accent' => 'dm-serif-text', 'font_size_base' => 18, 'font_weight_heading' => 600, 'font_preload' => 1]);
    $h = $curl('/');
    check('override en <head> con las 5 variables', strpos($h, '<style id="pswpt-typography-overrides">:root{--psw-font-body:var(--wp--preset--font-family--inter);--psw-font-heading:var(--wp--preset--font-family--system);--psw-font-accent:var(--wp--preset--font-family--dm-serif-text);--psw-font-size-base:18px;--psw-font-weight-heading:600;}</style>') !== false);
    check('preload del woff2 antes de las hojas', preg_match('#<link rel="preload" href="http://localhost:8888/WPfemsculpt/wp-content/uploads/fonts/inter-400\.woff2" as="font" type="font/woff2" crossorigin>.*<link rel=\'stylesheet\' id=\'pswpt-grid-css\'#s', $h) === 1);
    ...
    update_option('pswpt_settings', ['font_body' => 'inter', 'font_preload' => 0]);
    ...
    ob_start(); pswpt_settings_page_html(); $page = ob_get_clean();
    check('pestaña Tipografía con desplegables que listan theme + Biblioteca', strpos($page, 'id="content-typography"') !== false && preg_match('/<select id="pswpt_font_body"[^>]*>.*?<option value="dm-sans"[^>]*>DM Sans<\/option>.*?<option value="system"[^>]*>System · sin archivos<\/option>.*?<option value="inter"[^>]*>Inter · Biblioteca<\/option>/s', $page) === 1 && strpos($page, '<select id="pswpt_font_accent"') !== false && strpos($page, 'Apariencia → Fuentes') !== false);
} finally {
    ...
    if ($orig_settings === null) delete_option('pswpt_settings'); else update_option('pswpt_settings', $orig_settings);
