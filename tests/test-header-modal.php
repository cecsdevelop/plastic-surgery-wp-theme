 ...
 * GET pswpt/v1/modal?lang= con shortcodes resueltos en ese idioma.
 *
...
$rest = function (string $lang): array {
    $out = (string) shell_exec('curl -s -D - ' . escapeshellarg('http://localhost:8888/WPfemsculpt/wp-json/pswpt/v1/modal?lang=' . $lang));
    [$headers, $body] = explode("\r\n\r\n", $out, 2) + ['', ''];
    preg_match('/^HTTP\/\S+ (\d+)/', $headers, $m);
    return [(int) ($m[1] ?? 0), $headers, json_decode($body, true) ?: []];
};

$orig_header = get_option('pswpt_header_settings', null);
$form_id = 0; $comp_id = 0;
...
try {
    ...
    update_option('pswpt_header_settings', ['cta_type' => 'modal', 'cta_text' => ['es' => 'Hablemos', 'en' => "Let's talk"], 'cta_modal_title' => ['es' => 'Escribinos'], 'cta_modal_content' => '[modal-intro]<script>window.__crm = 1;</script>[form slug="contacto-modal"]']);
    ...
    check('botón y dialog vacío con data-cta-modal-src por idioma', strpos($es, '>Hablemos</button>') !== false && preg_match('#<dialog id="header-cta-modal"[^>]* data-cta-modal-src="http://localhost:8888/WPfemsculpt/wp-json/pswpt/v1/modal\?lang=es"#', $es) === 1 && preg_match('#data-cta-modal-src="[^"]*lang=en"#', $en) === 1);
    ...
    check('EN: 200 con formulario en inglés y componente en inglés', $code === 200 && strpos($body['html'] ?? '', '>Name <span class="pswpt-form__req"') !== false && strpos($body['html'], 'name="_lang" value="en"') !== false && strpos($body['html'], '<p class="intro">Previous · en</p>') !== false);
    ...
    update_option('pswpt_header_settings', ['cta_type' => 'url', 'cta_url' => 'https://x.com', 'cta_text' => ['es' => 'x'], 'cta_modal_content' => '<p>oculto</p>']);
    [$code, , $body] = $rest('es');
    check('CTA no-modal → 404 sin html', $code === 404 && ($body['html'] ?? 'x') === '');
} finally {
    if ($orig_header === null) delete_option('pswpt_header_settings'); else update_option('pswpt_header_settings', $orig_header);
