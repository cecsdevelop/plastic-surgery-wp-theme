...
try {
    ...
    check('cards: sección --cards, tarjeta con etiqueta, nombre h3, excerpt plegable, flecha y texto accesible', strpos($cards, '<section class="services section services--cards">') !== false && strpos($cards, '<div class="services__grid services__grid--cards">') !== false && strpos($cards, '<article class="services__tile"><a class="services__tile-link" href="http://localhost:8888/Intelindev/servicios/servicio-test-sections/">') !== false && strpos($cards, '<h3 class="services__tile-name">Servicio Test</h3><p class="services__tile-excerpt"><span>Resumen test</span></p><span class="services__tile-arrow" aria-hidden="true"></span><span class="screen-reader-text">Conocer más</span>') !== false);
    check('cards: sin imagen destacada queda el marcador; sin tarjetas de ícono', strpos($cards, '<span class="services__tile-image"></span>') !== false && strpos($cards, 'services__card') === false);
    ...
    check('CSS de Servicios: tarjetas con hover, video, stack (ítems vacíos ocultos), lista y detalle', strpos($css, '.services__tile:hover .services__tile-label,') !== false && strpos($css, '.video__player {') !== false && strpos($css, '.stack__item:has(dt:empty) { display: none; }') !== false && strpos($css, '.services-nav__link[aria-current="page"] {') !== false && strpos($css, '.service__inner {') !== false && strpos($css, '.hero--interior::before {') !== false && strpos($css, '.btn--phone::after {') !== false);
