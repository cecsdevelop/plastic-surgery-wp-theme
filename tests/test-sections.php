...
try {
    ...
    check('contact-info: 3 cajas enlazadas con íconos phone/mail/map-pin del theme, títulos por defecto (Dirección del diccionario) y datos', strpos($ci, '<section class="contact-info section" id="contact-info">') !== false && substr_count($ci, '<li class="contact-info__item">') === 3 && strpos($ci, '<a class="contact-info__link" href="https://wa.me/593945"><img class="contact-info__icon" src="' . get_template_directory_uri() . '/assets/img/icons/phone.svg"') !== false && strpos($ci, '/assets/img/icons/mail.svg') !== false && strpos($ci, '/assets/img/icons/map-pin.svg') !== false && strpos($ci, '<span class="contact-info__title">WhatsApp</span><span class="contact-info__text">+593 945</span>') !== false && strpos($ci, '<span class="contact-info__title">Dirección</span><span class="contact-info__text">Calle 1</span>') !== false && strpos($ci, 'href=""') !== false);
    check('contact-info EN: título por defecto traducido y caja sin dato vacía (se oculta por CSS)', strpos($render('[test-contact-info item1_text="x"]', 'en'), '<span class="contact-info__title">Address</span><span class="contact-info__text"></span>') !== false);

    echo "12) CSS e íconos\n";
    ...
    check('CSS de Servicios/Portafolio: tarjetas con hover, "ver más", video, stack (ítems vacíos ocultos), lista, detalles y ficha', strpos($css, '.tile:hover .tile__label,') !== false && strpos($css, '.tiles {') !== false && strpos($css, '.btn--plus::after {') !== false && strpos($css, '.project__inner {') !== false && strpos($css, '.project__fact dt::after { content: ":"; }') !== false && strpos($css, '.post-card {') !== false && strpos($css, '.pagination .page-numbers.current {') !== false && strpos($css, '.comments__form input[type="text"]') !== false && strpos($css, '.post-tools__share-icon {') !== false && strpos($css, '.contact-info__item:has(.contact-info__text:empty) { display: none; }') !== false && strpos($css, '.video__player {') !== false && strpos($css, '.stack__item:has(dt:empty) { display: none; }') !== false && strpos($css, '.services-nav__link[aria-current="page"] {') !== false && strpos($css, '.service__inner {') !== false && strpos($css, '.hero--interior::before {') !== false && strpos($css, '.btn--phone::after {') !== false);
    check('26 íconos SVG del diseño en assets/img/icons + logo', count(glob(dirname(__DIR__) . '/assets/img/icons/*.svg')) === 26 && file_exists(dirname(__DIR__) . '/assets/img/logo.svg'));
} finally {
